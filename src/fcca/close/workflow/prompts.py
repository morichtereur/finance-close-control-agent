...
#: Version of the prompt contract, recorded on every ``actor='model'`` trace
#: record. A prompt change is a change to how the system reasons, so it has to
#: be visible in the trace — otherwise two runs that differ only by prompt look
#: identical in the record. Bump this whenever SYSTEM_PROMPT or the payload
#: layout changes in a way that could move an answer.
PROMPT_VERSION = "close-v1"
