...
from fcca.close.evaluation.benchmark import (
    BenchmarkRun,
    collect_runs,
    run_benchmark,
    run_slug,
...
from fcca.close.presentation import console, render_benchmark
...
def main(argv: list[str] | None = None) -> int:
    ...
    for provider in providers:
        ...
        if run.metrics is not None:
            write_run_detail(
                run, settings.results_dir / f"{run_slug(run.provider, run.model)}.json"
            )
